...
- **JSON Viewer and Markdown Viewer are now one "File Viewer" window family.** Under the hood they share one spawn/registry/window-chrome implementation instead of two separate parallel ones — same pin/minimize/maximize/close, same `Win+Shift+J` switcher, same restore-on-restart behavior — the visible controls still differ by file type, but there's now one thing to grow into a broader file/code viewer over time. The standalone **"Open Markdown File…"** palette command is gone — pasting a `.md` path already opens it in the Markdown Viewer, so the dedicated command was redundant.
