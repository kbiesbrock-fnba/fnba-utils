...
New palette command **Clipboard** plus a dedicated `Win+V` global shortcut opens a Raycast-style clipboard history window. Built to replace the native Windows Clipboard (Win+V) with: unlimited SQLite-backed history, fuzzy search, source-app attribution, image thumbnails, pinned entries, and sensitivity-aware masking.
