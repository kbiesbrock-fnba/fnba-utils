...
- **Assume Identity on several connections at once.** In the connection picker, press **Space** to tick multiple connections (a checkbox appears on each; a running count shows at the top), then **Enter** to run the assume against every ticked connection in turn. The review screen lists all the connections you picked, the executing view shows per-connection progress (e.g. `2/3`), and the result screen stacks one outcome per connection — each with its own password and Copy button, or an inline error if that connection failed (one failure no longer stops the others). Not ticking anything still works exactly as before: Enter (or clicking a row, or its number key) assumes on just that one connection. On the results screen, **Enter closes** the palette and **Esc does nothing** — there's no accidental jump back to the picker anymore.
