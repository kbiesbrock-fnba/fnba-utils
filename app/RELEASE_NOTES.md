...
- **Assume Identity on several connections at once.** In the connection picker, press **Space** to tick multiple connections (a checkbox appears on each; a running count shows at the top), then **Enter** to run the assume against every ticked connection in turn. The review screen lists all the connections you picked, the executing view shows per-connection progress (e.g. `2/3`), and the result screen lists one outcome per connection — Success and Previously sit side-by-side in an aligned grid rather than stacked, so scanning several connections' before/after takes a glance instead of scrolling — each with its own password and Copy button, or an inline error if that connection failed (one failure no longer stops the others). The main palette window is wider now to fit that layout comfortably. Not ticking anything still works exactly as before: Enter (or clicking a row, or its number key) assumes on just that one connection. On the results screen, **Enter closes** the palette and **Esc does nothing** — there's no accidental jump back to the picker anymore.
...
- **SQL panels can switch connections in place.** A SQL Query window is no longer tied to the connection you opened it from — the server name in the header is now a dropdown. Run a query on meleagris, pick a different environment from the dropdown, and run the same query again without opening a new window (the previous result clears so it's never misattributed to the new server). Opening a connection from Mission Control now launches a fresh SQL panel each time (rather than reusing the one window per server), and pinned SQL panels remember whichever connection you last switched them to when they reopen on restart.
- **Saved SQL queries are now a folder of `.sql` files you control.** Set a folder in your config file — open `%LOCALAPPDATA%\fnba-utils\config.yaml` and add `sql_library:` with a `root:` pointing at a folder (a WSL path like `\\wsl$\Ubuntu\home\you\dev\sql` works) — and the SQL Query panel lists your saved queries as plain `.sql` files there, with subfolders showing up as collapsible headings. The panel's setup notice shows the exact config path and snippet with an **Open config** button, and picks up your edit as soon as you hit **Refresh** (no restart). Click a file to load it, edit, and **Ctrl+S** to save back; use **Save As…** to file a new query into any folder, **New folder** to organize, and Delete to remove. Your existing in-app saved queries are exported into the folder once, automatically, the first time the panel reads a configured folder (nothing is deleted from the old list). The panel refreshes on focus, and if WSL isn't running it shows a "library unavailable" notice with a Retry button instead of breaking — the query editor keeps working either way.
...
- **Pasting a path puts the obvious action first.** The palette now orders the file/folder actions by what you actually pasted, so Enter usually does the right thing without arrowing down: a text-ish file (`.sql`, `.log`, `.cs`, `Dockerfile`, `.bashrc`, …) leads with **Open in Notepad++**; a folder leads with **Show in Explorer** (and no longer offers Notepad++, which can't open a folder); anything else — PDF, xlsx, image, zip — leads with a new **Open** action that hands the file to whatever app owns that file type, exactly like double-clicking it in Explorer. **Open** is available for text files too, for when you want the `.csv` in Excel rather than in Notepad++. A `.md`/`.markdown` path instead leads with **Open in Markdown Viewer** and a `.json`/`.jsonc` path leads with **Open in JSON Viewer**, loading the file straight into the app's own viewer rather than Notepad++. Paths copied with Explorer's **Copy as path** (Ctrl+Shift+C) are also recognized now — the surrounding quotes used to stop the palette from seeing a path at all.
...
- **Clipboard: paste with label.** For an entry you've given a custom label, **Ctrl+L** (or the right-click menu's **Paste with label**) pastes `label: content` instead of just the content — handy for pasting a value somewhere its meaning isn't obvious from the text alone. For a sensitive entry, **Ctrl+Shift+L** does the same but with the original (reveal-token gated) content rather than the obfuscated version. Only active when the entry actually has a label.
- **Clipboard window opens wider and ready to type.** It now sizes itself to half the width of whichever monitor it's on (instead of a fixed, cramped 540px) and reliably grabs keyboard focus in the search box the moment `Win+V` shows it, so you can start typing a filter immediately. **Shift+Enter** now pastes the original/sensitive content directly (previously this required `Ctrl+Shift+Enter`, which still works too).
...
- **JSON Viewer and Markdown Viewer are now one "File Viewer" window family.** Under the hood they share one spawn/registry/window-chrome implementation instead of two separate parallel ones — same pin/minimize/maximize/close, same `Win+Shift+J` switcher, same restore-on-restart behavior — the visible controls still differ by file type, but there's now one thing to grow into a broader file/code viewer over time. The standalone **"Open Markdown File…"** palette command is gone — pasting a `.md` path already opens it in the Markdown Viewer, so the dedicated command was redundant. **New:** JSON Viewer now has the same **Open / Save / Save As** file-backed editing Markdown Viewer already had — dirty-tracking, an amber banner when the bound file changes on disk, and a save-prompt before closing with unsaved changes — plus a **Preview / Edit / Split** layout toggle as its first toolbar row (its existing Format/Tree/Flatten/Schema/Diff/A-Z buttons moved to a second row), matching Markdown Viewer's toolbar shape. Both viewers' search is now hidden until you press **Ctrl+F**, which reveals a find bar — Markdown Viewer's is brand new (jumps to and selects each match, with a "N of M" counter and next/prev), JSON's is the same search it always had, just tucked away until summoned.
- **Markdown Viewer** (📝, search "markdown"): paste or type Markdown in the palette (or prefix with `md `), or use the **Markdown Viewer** palette command to open a blank one. Each window has a **Preview / Edit** toggle — Preview renders headings, tables, code blocks, links, and lists on a dark theme; Edit gives you a plain textarea. Links in the preview open in your default browser. Open Markdown Viewer windows now appear alongside JSON Viewer windows in the **`Win+Shift+J`** switcher. Windows, their content, position, and mode survive restarts just like JSON Viewer windows. Open `.md` files from disk via the **📂 Open** toolbar button / **Ctrl+O**; save with **💾 Save** / **Ctrl+S** or **Save As…** / **Ctrl+Shift+S** — the window title shows the filename and a **●** when there are unsaved changes; closing a window with unsaved edits now prompts to save, discard, or cancel. When the file is edited by another program or a git operation while the window is open, an amber banner appears on re-focus offering **Reload** (discards local edits), **Open disk copy ↗** (opens the on-disk version in a new unbound window so you can compare), or **Keep mine** (dismisses without clobbering your work). If the file was deleted on disk, the banner instead offers **Save again** to recreate it.
...
- **JSON Viewer windows are now real, independent windows** -- every viewer you open appears in the taskbar and Alt-Tab with its own title bar: drag, snap (Win+Arrow / FancyZones), maximize, minimize, pin on top (pin button), or go fullscreen (F11). Esc or the close button (X) closes just that window.
- **Win+Shift+J is now a JSON window switcher** -- pops a list of your open JSON viewers (most recent first) plus a "New window" option; with none open it simply opens a fresh viewer.

- **JSON Viewer: open several at once, clear, and close.** You can now keep multiple JSON Viewer windows open side by side — the palette's **Open in New Window** button (and the **＋ New** button in the toolbar) each spawn a fresh window, so you can compare or work on different blobs at the same time. A **🗑 Clear** button resets the current window's input and search in one click, and an **✕** button (or **Esc**) closes a window. The viewer also opens in **Format** mode by default.
- **JSON Viewer command.** A new **JSON Viewer** palette command (📋 icon, search "JSON") opens a dedicated window (`Win+Shift+J`) where you can paste JSON and explore it via an interactive tree view with expand/collapse, path-based search/filtering, and multiple copy formats. The tree renders nested objects and arrays with automatic summaries; click any node to select it. The copy dropdown offers: prettified (2-space), minified, JSONPath format (`$.path.to.node`), JSONPath with array wildcards (`$.items[].name`), node value only, or the entire branch rooted at the selected node. Additional modes flatten JSON to dot-notation key=value pairs, auto-generate a JSON Schema, or diff two JSON blobs side-by-side. Use **Sort A-Z** to recursively reorder all object keys alphabetically.

- **"Next up" stars on To Do items.** The standup preview's **To Do** section now has a star next to each item — think of it as *"these are the next things I'm considering working on."* The Teams post and **📋 Copy** report only include starred items from To Do; unstarred items stay in the preview (dimmed and struck through) so you can still see your full backlog without dumping it into the daily summary. To Do items start **unstarred**, and the system **auto-stars up to 3 each preview** (highest priority + nearest due date first) so you don't start from zero. Your picks **carry over day to day** — yesterday's stars are honored, and if a starred item moves out of To Do, the auto-promoter only tops you back up to 3 when you fall below. Other groups (In Progress, In Review, Done) still post in full.
- **Label and edit clipboard entries.** Each clipboard row can now carry a custom **label** that shows above the captured text in the list — handy for finding a specific copy among hundreds of similar ones. Open an entry in `Win+V`, type a label in the new field at the top of the detail panel, and press Enter or click away to save. Labels are also searchable. A new **✎ Edit** button next to the content lets you rewrite the captured text in place — pasting that entry afterward pastes the edited version. Edit is disabled for image entries and for entries flagged sensitive (clear the sensitive flag first).
- **Manually mark clipboard entries as sensitive (or clear false positives).** The sensitive badge in the detail panel is now a toggle: click **mark sensitive** on a benign-looking entry whose PII wasn't auto-detected, and the regular paste path switches to a masked safe version (`Ctrl+Shift+Enter` still pastes the original). Click **sensitive ✓** on a false-positive to clear the flag and restore plain pasting.
...
- **Assume Identity now searches the whole directory.** Instead of picking from a hand-maintained list, just start typing a name or username and the user picker looks everyone up live (on meleagris) — you can become any user, not only the preconfigured ones.
- **Find someone by the right they hold.** In the user picker, press → to switch from **People** to **Rights**: type a right by name *or by its ID number*, pick it, and the people who have it appear as assume candidates (← or Esc steps back). And to go the other way — "what can this person do?" — press **Tab** on anyone in the results to see the full list of rights they hold.
- **Favorites + Recently Used, both numbered.** Favorites are your pinned list (curated, persisted, shareable); Recently Used holds the last 5 *unpinned* assumes (transient — they cycle out as new ones come in). Both sections share one 1–9 hot-pick sequence: favorites first, then recents fill the remaining digits. Favorites re-rank by recency-of-use. Press **Delete** on either to drop it (including the shipped defaults you don't actually use). Connections still use a fixed per-environment number.
- **Pinning is explicit — no more accidental favorites.** Assumes never auto-prompt to pin. Anything you assume that isn't already a favorite drops into Recently Used. To make it stick, click the **pin** button on the recent row — a label prompt pops up with the role pre-filled (edit or press Enter to save). Newly-pinned favorites jump straight to #1 in Favorites and disappear from Recents.
- **Dropped the "custom" tag** on user-pinned favorites — they're just favorites now, indistinguishable from the shipped defaults.
- **Right Lookup is now part of Assume Identity.** The separate "Right Lookup" command has been removed — finding a right's holders and auditing a person's rights both live inside the renamed **Assume Identity (Search by Rights)** command, so you go straight from a right to assuming one of its holders. (Assuming someone found this way now also targets their real Windows login, fixing a case where a mismatched nickname could silently switch to the wrong account.)
- **Directory search shows everyone.** People without a Windows login row (contractors, archived associates, service accounts) used to silently vanish from name search; they're back, rendered as "no login" entries. The same fix surfaces orphan permission rows — `user_groups` entries whose associate record was deleted — when you audit who holds a right, instead of dropping them.
- **Already-favorited people appear in directory results.** Searching for someone you've pinned under one label now still shows them as a directory hit, so you can pin the same person again under a different role context without backing out of the search first.
- **Login picked is the most-recently-modified.** When an associate has multiple `domain_username` rows (legacy + current, contractor + permanent), the picker assumes the most-recently-touched one — matching what an interactive sign-in would resolve to. Previously the alphabetically-smallest login won, which could be a disabled legacy account.
- **Search punctuation is literal.** `_`, `%`, `[`, and `\` in the search box now match themselves, not SQL wildcards. A query like `_kbiesbrock` filters by literal underscore prefix; pasting a name like `Smith [Contractor]` returns that row instead of zero results.
- **Digits typed mid-search reach the input.** Names or logins containing digits (`jdoe2`, `k1m`, …) are no longer hijacked by the 1–9 hot-pick handler. Hot-picks still work when the search box is empty.
- **Removing a shipped default favorite is reversible.** Click × on a default (or press Delete), then later pin the same person under the same label, and the entry comes back. Previously a hidden default stayed invisible until you hand-edited `assumeIdentity.json`.
- **Crash-safe favorites file.** Pin / unpin / mark-used writes go via a temp file + atomic rename, so a crash, lockscreen, antivirus interception, or power loss mid-write can't leave `assumeIdentity.json` half-written (and the feature bricked until manual cleanup).
- **Right selection matches by ID.** Two rights that happen to share the same display name no longer have their holders silently merged into one list — the picker now uses the right's `right_id` (the canonical primary key) when drilling in.
- **No success-to-error flicker; stale drills don't overwrite the current view.** A transient database hiccup during the post-assume background data refresh can no longer flip a just-completed assume back to an error screen. And drilling a slow right (or tabbing one person's rights) then quickly switching to another no longer lets the late response clobber the new view's data.
- **Recents with blank role labels show their buttons.** If an associate's `job_title` was empty in the database, the recents row's pin / × buttons used to go missing (an empty-string label was treated as falsy). The label now falls back to department / "Custom" as intended.
- **Corrupt local-storage doesn't crash the palette.** A junk `fnba-utils:recent-users` / `recent-rights` payload (from a devtools edit or older schema) now resets to empty instead of crashing the command palette at module load.
