...
- **Saved SQL queries are now a folder of `.sql` files you control.** Point the SQL Query panel at a folder — a WSL path like `\\wsl$\Ubuntu\home\you\dev\sql` works, or pick one with the folder browser — and your saved queries become plain `.sql` files there, with subfolders showing up as collapsible headings. Click a file to load it, edit, and **Ctrl+S** to save back; use **Save As…** to file a new query into any folder, **New folder** to organize, and Delete to remove. Your existing in-app saved queries are exported into the folder once, the first time you set it up (nothing is deleted from the old list). You can change the folder later from the panel header, and if WSL isn't running the panel shows a "library unavailable" notice with a Retry button instead of breaking — the query editor keeps working either way.
